...
class DatabricksOAuthAttemptTest {
    ...
    @Test
    fun `fresh attempts use URL-safe random state and verifier`() {
        var seed = 0
        val created =
        ...
        assertTrue(created.state.matches(Regex("[A-Za-z0-9_-]+")))
        assertTrue(created.verifier.matches(Regex("[A-Za-z0-9_-]+")))
        assertEquals(43, created.state.length)
        assertEquals(43, created.verifier.length)
    }
}
