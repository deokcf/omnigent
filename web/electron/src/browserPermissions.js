...
function registerBrowserPermissions(session, { canPrompt, showPrompt, store }) {
  ...
  function ask(ctx, reload) {
    ...
    const request = { ...ctx, controller: new AbortController() };
    pending = request;
    request.result = Promise.resolve()
      ...
      .then((choice) => {
        ...
        if (choice === "always-allow" || choice === "deny") {
          ...
          if (store.get(ctx.origin) !== undefined) store.set(ctx.origin, undefined);
          visits.set(ctx.origin, { granted: true, revision: store.revision(ctx.origin) });
        } else {
